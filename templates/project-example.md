...
> **脱敏提示**：本模板用于公开 GitHub 仓库。填写前请阅读 [extract-experience.md 第 7.5 步](../commands/extract-experience.md) 的脱敏 gate。涉及真实项目时，背景字段一律使用泛化描述，禁止出现部门/公司/内部用户群/团队规模等可识别信息。
...
**目标用户**：[泛化描述，如"开发者 / 学生 / 公众"；禁止具体部门或内部用户群]

**核心问题**：[解决什么问题]

**项目规模**：[代码量、开发周期；团队规模可选，避免暴露组织结构]
...
- **已检查项**：
  - [ ] 识别类（部门/公司/用户群/团队规模）
  - [ ] 技术类（API/域名/内部服务/包名/文档链接）
  - [ ] 数据类（业务数据/日志/表名字段名）
  - [ ] 个人信息（邮箱/手机/工号/姓名/社交链接）
- **残留风险**：[无 / 说明]
- **占位符使用记录**：[如 `[部门]`、`api.example.com` 等，列出替换项]
