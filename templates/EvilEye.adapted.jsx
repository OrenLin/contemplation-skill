// EvilEye — Canonical adapted shader component
// Reference implementation showing ALL fixes from this skill applied:
//   - gl_FragCoord normalization (not vUv)
//   - uResolution as vec3 with device pixels
//   - uScale uniform for zoom control
//   - pointer + touch tracking in uv space
//   - proper resize using getBoundingClientRect + device pixels
//
// Source: adapted from reactbits.dev EvilEye, normalized for mobile.
...
// Texture-based noise — better performance and detail than procedural FBM.
...
uniform float uTime;
uniform vec3 uResolution;       // (width, height, aspect) — device pixels
uniform sampler2D uNoiseTexture;
uniform float uPupilSize;
uniform float uIrisWidth;
uniform float uGlowIntensity;
uniform float uIntensity;
uniform float uScale;           // zoom control — KEY for mobile
...
void main() {
  // CRITICAL: gl_FragCoord normalization, y as reference axis.
  // Auto-adapts to any aspect ratio. No contain/cover branching.
...
export default function EvilEye({
  ...
  irisWidth = 0.3,
  glowIntensity = 0.6,
  scale = 0.5,             // tuned for portrait mobile — eye ~50% screen height
  noiseScale = 1.3,
  pupilFollow = 1.1,
  flameSpeed = 0.8,
  ...
  useEffect(() => {
    ...
    // Pointer/touch tracking — convert to -1..1 uv space.
    ...
    // CRITICAL: resize uses device pixels (gl.canvas.width/height), not CSS pixels.
    function resize() {
