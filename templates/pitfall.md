...
> 若本踩坑记录的代码示例、错误日志、配置片段来自真实项目，必须执行 [extract-experience.md 第 7.5 步](../commands/extract-experience.md) 的脱敏 gate：
> - 内部 API/域名 → `https://api.example.com`
> - 内部包名/路径 → 占位符（`@scope/pkg`）
> - 真实日志/报错 → 去除用户标识与内部路径
> - 业务数据样例 → 替换为合成数据
>
> 已脱敏：[是/否]　审查人：[Agent/姓名]　日期：[YYYY-MM-DD]
